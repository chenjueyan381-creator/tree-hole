...
这个项目有两个境外依赖，**任意一个不通，站点就用不了**。两者在大陆的处境并不一样：

| 依赖 | 大陆可访问性 |
| --- | --- |
| `*.github.io`（GitHub Pages） | **通常可以访问**。很多国内开发者的博客就放在 github.io 上。历史上有过 DNS 污染和限速的时段，速度也看运营商，但不是常态封锁 |
| `*.vercel.app`（Vercel） | **基本被 DNS 污染**，大陆多数网络打不开。这是本项目实际遇到的问题 |
| `*.supabase.co`（数据层） | **不稳定，需要实测**。不像 vercel.app 那样确定，但也谈不上可靠 |

所以如果你的目标是让国内能访问，**优先试 GitHub Pages**（见上一节），它比 Vercel 靠谱得多。

### 怎么分辨是哪一层不通

- 页面压根打不开（浏览器报「网址无法打开」）→ **前端域名**不通
- 页面能打开、但一直卡在「加载中…」或显示「加载失败」→ 前端通了，**Supabase** 不通

单独测 Supabase（国内网络、不开代理，浏览器直接打开）：

```
https://rumdtkhukjiyegagmrbj.supabase.co/rest/v1/
```

返回一段 JSON 报错（提示缺 apikey）= 通的；转圈或连不上 = 不通。

### 如果 Supabase 也不通

| 方案 | 代价 |
| --- | --- |
| 香港/新加坡 VPS 上用 docker-compose 自建 Supabase | ¥40-100/月；**不需要 ICP 备案**；前端只需改 `.env` 里那两个变量 |
| 国内云（阿里云/腾讯云） | 最快，但必须 ICP 备案（1-3 周）；匿名 UGC 站点在合规上有额外要求 |
